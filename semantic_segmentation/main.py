...
def parse_args():
    ...
    parser.add_argument('--root', type=str, default='../../../Dataset/S3DIS/s3disfull/raw/', help='data root')
...
def main(args):
    ...
    #     misc.set_random_seed(args.seed + int(args.gpu), deterministic=False)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            log_string('[mAcc] eval point avg class acc: %f' % (
                np.mean(np.array(total_correct_class) / (np.array(total_seen_class, dtype=np.float64) + 1e-6)) * 100.0))
