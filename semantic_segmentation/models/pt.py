...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        # self.reduce_dim = nn.Linear(self.encoder_dims,  self.trans_dim) if self.trans_dim != self.encoder_dims else None
    ...
    def load_model_from_ckpt(self, bert_ckpt_path, model_key='ACT_encoder'):
        if bert_ckpt_path is not None:
            ...
            for k in list(base_ckpt.keys()):
                if k.startswith(model_key):
                    ...
                    del base_ckpt[k]
                elif k.startswith('MAE_encoder') :
                    base_ckpt[k[len('MAE_encoder.'):]] = base_ckpt[k]
                    del base_ckpt[k]
                elif k.startswith('mask_encoder') :
    ...
    def forward(self, pts):
        ...
        # group_input_tokens = self.reduce_dim(group_input_tokens) if self.reduce_dim is not None else group_input_tokens
