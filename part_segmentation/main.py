...
def parse_args():
    ...
    parser.add_argument('--ckpts', type=str, default='../experiments/pretrain/pretrain/BR_DCenter_Q4_DPatch_Q2/ckpt-last.pth', help='ckpts')
    parser.add_argument('--root', type=str, default='../../../Dataset/Shapenet/', help='data root')
...
def main(args):
    ...
    '''LOG'''
    args = parse_args()
    logger = logging.getLogger("ShapeNetPart")
    ...
    root = os.path.join(args.root, 'shapenetcore_partanno_segmentation_benchmark_v0_normal')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            for cat in sorted(shape_ious.keys()):
                log_string('eval mIoU of %s %f' % (cat + ' ' * (14 - len(cat)), shape_ious[cat] * 100.0))
        ...
        log_string('Epoch %d ETA: %s test Accuracy: %f  Class avg mIOU: %f  Inctance avg mIOU: %f' % (
            epoch + 1, eta_str, test_metrics['accuracy'], test_metrics['class_avg_iou'], test_metrics['inctance_avg_iou']))
        ...
        log_string('[Acc.] Best accuracy is: %.5f' % (best_acc))
        log_string('[mIoU_C] Best class avg mIOU is: %.5f' % (best_class_avg_iou))
        log_string('[mIoU_I] Best inctance avg mIOU is: %.5f' % (best_inctance_avg_iou))
        global_epoch += 1
