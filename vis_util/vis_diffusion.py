...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        neighborhood = neighborhood - center.unsqueeze(2)  # 相对坐标
...
def process_point_cloud(pts, num_group=64, group_size=32, mask_ratio_rand=0.6, mask_ratio_block=0.6, T=2000):
    ...
    neighborhood, center = grouper(pts)  # neighborhood: (B, G, M, 3), center: (B, G, 3)
    ...
    for t_min, t_max in timestep_ranges:
        ...
        save_point_cloud_ply(x_t_vis_rand_t[0], os.path.join(output_dir, f"./diffusion/rand_visible_center_noised_{range_str}.ply"))
        save_point_cloud_ply(x_t_msk_rand_t[0], os.path.join(output_dir, f"./diffusion/rand_masked_center_noised_{range_str}.ply"))
        save_point_cloud_ply(x_t_msk_points_rand_t[0], os.path.join(output_dir, f"./diffusion/rand_masked_point_cloud_noised_{range_str}.ply"))
        save_point_cloud_ply(x_t_vis_block_t[0], os.path.join(output_dir, f"./diffusion/block_visible_center_noised_{range_str}.ply"))
        save_point_cloud_ply(x_t_msk_block_t[0], os.path.join(output_dir, f"./diffusion/block_masked_center_noised_{range_str}.ply"))
        save_point_cloud_ply(x_t_msk_points_block_t[0], os.path.join(output_dir, f"./diffusion/block_masked_point_cloud_noised_{range_str}.ply"))
